...
export default function OrderDetail({ id }: { id: string }) {
  ...
  const [valueAddressId, setValueAddressId] = useState<DeliveryAddress | null>(
    null
  );
  ...
  const [modalOpenConfirmPayment, setModalOpenConfirmPayment] =
  ...
  useEffect(() => {
    ...
    const fetchProduct1 = async () => {
      const res = await getDeliveryAddressByOrderId({
        orderId: data?.deliveryAddressId,
      });
      if (res.code !== -1) setValueAddressId(res.address);

      const res1 = await getPaymentById(data?.paymentId);
      if (res1.code === 1) setValuePaymentId(res1.payment);

      const res2 = await getUserProfileById(data?.profileId);
      if (res2.code === 1) setValueProfileId(res2.account);
    };
      ...
      <div className="container mx-auto px-15 pb-10 space-y-4">
        ...
        <div className="flex justify-between border px-4 py-5 rounded-xl shadow-md mb-6">
          <div>
            <div className="text-2xl flex gap-3">
              ...
              {valuePaymentId?.paymentStatus === "PAID" || !data.paymentId ? (
                ...
                <Button
                  onClick={() => setModalOpenConfirmPayment(true)}
                  className="inline-flex items-center gap-1 rounded-full bg-gradient-to-r from-red-400 to-red-500 text-white text-sm font-semibold px-3 py-1 shadow-md"
                >
                  chưa thanh toán
                </Button>
              )}
          ...
          <Button
            ...
            className="rounded-md bg-amber-500 hover:bg-amber-500/85"
          >
            Xuất đơn
          </Button>
        </div>
                ...
                <div className="overflow-x-auto rounded-lg border border-gray-200">
                  <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y ">
                      {data.orderItemResponses.length > 0 &&
                        data.orderItemResponses.map((data, index) => (
                          <tr
                            key={index}
                ...
                <div className="w-full px-3 mx-auto">
                  ...
                  <div className="font-semibold space-y-2">
                    ...
                    <div className="flex justify-between border-b py-2 mb-5 text-gray-600">
                      ...
                      <span>
                        {formatMoney(
                          data.orderItemResponses.reduce(
                            (sum, p) => sum + p.price,
                            0
                          )
                        )}
                  ...
                  <div className="w-full px-3 mx-auto">
                    ...
                    <div className="font-semibold space-y-2 text-gray-600">
                      <div className="flex justify-between border-b py-2 mb-2">
                        <span className="flex items-center gap-2">
                          <ReceiptText className="w-5 h-5" /> Phương thức thanh
                          toán
                        </span>
                      ...
                      <div className="flex justify-between border-b py-2 mb-2">
                        ...
                        <span>
                          {formatMoney(Number(valuePaymentId?.amount))}
                        </span>
                      </div>
                ...
                <div className="bg-gray-50 border border-green-300 p-4 rounded-xl flex items-center gap-2">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <p className="font-medium text-sm">
                      {valueAddressId?.street},{valueAddressId?.ward},
                      {valueAddressId?.district},{valueAddressId?.province}
                    </p>
                  </div>
                </div>
...
const headerTitleTable = ["Mã sản phẩm", "Tên sản phẩm", "Đơn giá", "Số lượng"];
      ...
      <div className="bg-white rounded-lg shadow-lg w-[400px] p-5 relative">
        ...
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <p className="text-gray-600 text-sm">
            ...
            <span className="font-semibold"> #{id}</span> đã được thanh toán và
            giao hàng thành công không?
          </p>

          <div className="flex gap-3 mt-4">
            <button onClick={close} className="px-4 py-2 rounded-md border">
              Hủy
            </button>
